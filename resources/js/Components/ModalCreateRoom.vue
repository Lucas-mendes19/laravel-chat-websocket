<template>
    <!-- Main modal -->
    <div v-if="show" id="crypto-modal" tabindex="-1"
        class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0
            h-[calc(100%-1rem)] max-h-full flex justify-center bg-gray-900 bg-opacity-50 dark:bg-opacity-80 fixed inset-0 z-40">
        <div class="relative w-full max-w-md max-h-full m-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form @submit.prevent="createRoom()" class="p-6">
                    <p class="text-sm font-normal text-gray-500 dark:text-gray-400">Select the user(s) to be inserted into the room.</p>
                        ...
                        <ul class="my-4 space-y-3" v-for="user in users" :key="user.id">
                            <li>
                                <a href="#" class="flex items-center p-3 text-base font-bold text-gray-900 rounded-lg bg-gray-50 hover:bg-gray-100 group hover:shadow dark:bg-gray-600 dark:hover:bg-gray-500 dark:text-white">
                                    <input v-model="checkedUsers" :id="user.id" :value="user.id"
                    ...
                    <div>
                        <input v-model="name" type="text" id="success" autocomplete="off"
